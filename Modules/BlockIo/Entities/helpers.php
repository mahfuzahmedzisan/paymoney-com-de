    ...
    function updateAssetCredentials()
    {
        ...
        if (!empty($cryptoAssetSettings)) {
            ...
            foreach ($cryptoAssetSettings as  $cryptoAssetSetting) {
                ...
                $accountBalance = getBlockIoAccountBalance($networkCredential);
                
                if ($accountBalance === null) {
                    break;
                }

                $blockIoNetworkArray['account_balance'] = $accountBalance;
...
if (!function_exists('getBlockIoAccountBalance')) {
    function getBlockIoAccountBalance($networkCredential)
    {
        $blockIoBalance = (new \Modules\BlockIo\Classes\BlockIo())->getBlockIoAccountBalance($networkCredential->api_key, $networkCredential->pin);

        if (is_numeric($blockIoBalance)) {
            return $blockIoBalance->available_balance;
        }
