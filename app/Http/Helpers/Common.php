...
class Common
{
    ...
    */
    ...
    {
        $data['qrCode'] = $qrCode = QrCode::where(['object_id' => $id, 'object_type' => $objectType, 'status' => 'Active'])->first(['qr_image', 'secret']);
        if (empty($qrCode)) {
        ...
        if ($objectType == 'standard_merchant' || $objectType == 'express_merchant') {
            return generatePDF('user.merchant.qrCodePDF', 'merchant_qrcode_', $data);
        } else if ($objectType == "user") {
            return generatePDF('user.profile.qrCodePDF', 'customer_qrcode_', $data);
        }
    }
