...
$(document).ready(function(){
    ...
    $('.pad a[name=edit]').bind('click', function(){        
        if ($(this).find('img').attr('disabled') == 'disabled') {           
            ...
            $.ajax({
                type: 'POST',
                url: '/finance/bonus/collection/edit/id/'+$('.t6 tr.items input[type=checkbox]:checked').val(),         
        ...
        if ($(this).find('img').attr('disabled') == 'disabled') {           
        	...
        	if(confirm('Вы действительно хотите удалить выбранные записи?'))
        	{
        		...
        		$('.t6 tr.items input[type=checkbox]:checked').each(function(i, item) {
       			...
       			$.ajax({
               		type: 'POST',
               		url: '/finance/bonus/collection/delete',
               		data: data,        
               		success: function(result){
    ...
    $('.pad a[name=add]').bind('click', function(){        
		$('.loading').fadeIn(150);
		$('.form').load('/finance/bonus/collection/add');
		$('.form').fadeIn(500);
		$('.loading').html('').fadeOut(50);
	});
    ...
    $('.t6 tr.items a[href=#edit]').bind('click', function(){
        ...
        $.ajax({
            type: 'POST',
            url: '/finance/bonus/collection/edit/id/' + $(this).parents('tr').find('input[type=checkbox]').val(),
            success: function(result){
                $('.form').html(result);
                $('.form').fadeIn(500);
                $('.loading').fadeOut(50);
                    ...
                    <div class="container">
                        <div class="content">
                            <div class="main">
								...
								<!-- BEGIN FORM -->                                
								<div class="desc">
								    <h2>Управление коллекциями бонусов</h2>
								    <p>В этом модуле вы сможете управлять коллекциями бонусов. </p>
								</div>
								<div class="pad">
								    <div class="button">
                                ...
                                <div class="tabs">
                                    <ul>
                                        <li><a href="/finance/bonus">Бонусы</a></li>
								    ...
								    <div class="in-bg">
								        <table border="0" cellpadding="0" cellspacing="0" class="t6">
								        	<tr class="sort">
								                ...
								                <th>
								                	<?php if (strpos(url::current(),'/sort/up,count_bonus'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="down,count_bonus">Количество&nbsp;<img src="/media/images/up.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	elseif (strpos(url::current(),'/sort/down,count_bonus'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,count_bonus">Количество&nbsp;<img src="/media/images/down.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	else
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,count_bonus">Количество</a>
                                                	<?php
                                                	}
                                                	?>
                                                </th>
								            ...
								            <?php foreach ($data->list_collection as $row): ?>
												<tr class="items<?php if ($row === end($data->list_collection)) echo ' last' ?>">
								                    ...
								                    <td><a href="/finance/bonus/filter/0,<?php echo $row->id ?>,0,0,0">Всего&nbsp;<?php echo $row->count_bonus ?></a></td>
								                </tr>
											<?php endforeach ?>												
								            </table>		
