...
$(document).ready(function(){
    ...
    $('.filter input[type=button]').bind('click', function(){
        document.location.href = '/<?php echo eregi_replace('/filter/([0-9]+,[0-9]+,[0-9]+,[0-9]+,[0-9]+)', '', url::current()) ?>/filter/'+$('.filter').find('select[name=user_id]').val()+','+$('.filter').find('select[name=status]').val()+','+$('.filter').find('select[name=day]').val()+','+$('.filter').find('select[name=month]').val()+','+$('.filter').find('select[name=year]').val();
    });
