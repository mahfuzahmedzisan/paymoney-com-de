...
$(document).ready(function(){	
	...
	$('form').bind('submit', function(){
		...
		var filter_coin_size = /([0-9]+\.[0-9]{2}$)/;
		var string_coin_size = $('input[name=setting[coin_size]]').val();
		if (!filter_coin_size.test(string_coin_size) && string_coin_size != null)
	    {
			$('input[name=setting[coin_size]]').parent().find('span').removeClass('hide').html('Неправильный формат').addClass('up-box2');
			flag = true;
		}
		if (flag) {
                ...
                <td class="w01">
                    ...
                    <p><?php echo Kohana::lang('games.banking.'.$row->type.'.title') ?><br />
                    	...
                    	<?php foreach ($data->list_banking as $row1): ?>
                    		...
                    		<option <?php if ($row->banking_id === $row1->id) echo 'selected="selected"' ?> value="<?php echo $row1->id ?>"><?php echo $row1->title ?> - <?php number_format($row1->balance, 2, '.', '') ?></option>
                    		<?php endif ?>
                    	<?php endforeach ?>
                    	</select>
