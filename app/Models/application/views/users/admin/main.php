...
$(document).ready(function(){
    ...
    $('.pad a[name=edit]').bind('click', function(){        
        if ($(this).find('img').attr('disabled') == 'disabled') {           
            ...
            $.ajax({
                type: 'POST',
                url: '/users/admin/edit/id/'+$('.t6 tr.items input[type=checkbox]:checked').val(),         
    ...
    $('.t6 tr.items a[href=#edit]').bind('click', function(){
        ...
        $.ajax({
            type: 'POST',
            url: '/users/admin/edit/id/' + $(this).parents('tr').find('input[type=checkbox]').val(),
            success: function(result){
                $('.form').html(result);
                $('.form').fadeIn(500);
                $('.loading').fadeOut(50);
                        ...
                        <div class="content">
                            <div class="main">
								...
								<div class="desc">
								    ...
								    <p>В этом модуле вы сможете управлять администраторами. </p>
								</div>
								<div class="pad">
								    <div class="button">
								        ...
								        <table border="0" cellpadding="0" cellspacing="0" class="t6">
								        	<tr class="sort">
								                ...
								                <th>
                                                	...
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="down,username">Имя (Логин)&nbsp;<img src="/media/images/up.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	elseif (strpos(url::current(),'/sort/down,username'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,username">Имя (Логин)&nbsp;<img src="/media/images/down.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	else
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,username">Имя (Логин)</a>
                                                	<?php
                                                	}
                                                	?>
								                </th>								                
								            ...
								            <tr class="filter">
								                <td></td>
								                <td><input type="text" name="uname" value="<?php if (uri::segment('filter')) {$array = explode(",", uri::segment('filter')); if ($array[0]) echo $array[0];}?>"></td>
								                <td><input type="text" name="email" value="<?php if (uri::segment('filter')) {$array = explode(",", uri::segment('filter')); if ($array[1]) echo $array[1];}?>"><input type="button" value="Фильтр" /></td>
								            </tr>							            
								        	<?php foreach ($data->list_admin as $row): ?>
												<tr class="items<?php if ($row === end($data->list_admin)) echo ' last' ?>">
