...
$(document).ready(function(){
    ...
    $('.pad a[name=edit]').bind('click', function(){        
        if ($(this).find('img').attr('disabled') == 'disabled') {           
            ...
            $.ajax({
                type: 'POST',
                url: '/users/player/edit/id/'+$('.t6 tr.items input[type=checkbox]:checked').val(),         
    ...
    $('.t6 tr.items a[href=#edit]').bind('click', function(){
        ...
        $.ajax({
            type: 'POST',
            url: '/users/player/edit/id/' + $(this).parents('tr').find('input[type=checkbox]').val(),
            success: function(result){
                $('.form').html(result);
                $('.form').fadeIn(500);
                $('.loading').fadeOut(50);
    ...
    $('.filter input[type=button]').bind('click', function(){
        document.location.href = '/<?php echo eregi_replace('/filter/(.*),(.*)', '', url::current()) ?>/filter/'+$('.filter').find('input[name=uname]').val()+','+$('.filter').find('input[name=email]').val();
    });
                        ...
                        <div class="content">
                            <div class="main">
								...
								<div class="desc">
								    ...
								    <p>В этом модуле вы сможете управлять игроками. </p>
								</div>
								<div class="pad">
								    <div class="button">
								    ...
								    <div class="in-bg">
								        <table border="0" cellpadding="0" cellspacing="0" class="t6">
								        	<tr class="sort">
								                ...
								                <th>
								                	<?php if (strpos(url::current(),'/sort/up,username'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="down,username">Имя&nbsp;<img src="/media/images/up.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	elseif (strpos(url::current(),'/sort/down,username'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,username">Имя&nbsp;<img src="/media/images/down.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	else
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,username">Имя</a>
                                                	<?php
                                                	}
                                                	?>
                                                </th>								                
								                <th>
								                	<?php if (strpos(url::current(),'/sort/up,email'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="down,email">Email&nbsp;<img src="/media/images/up.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	elseif (strpos(url::current(),'/sort/down,email'))
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,email">Email&nbsp;<img src="/media/images/down.gif" alt="#"></a>
                                                	<?php
                                                	}
                                                	else
                                                	{
                                                	?>
                                                		<a href="javascript:;" rel="up,email">Email</a>
                                                	<?php
                                                	}
                                                	?>
                                                </th>
								                ...
								                <th>Проиграл</th>
								                <th>Баланс</th>
								                <th>Запросов на выплату</th>
								            </tr>	
								            <tr class="filter">
								                <td></td>
								                <td><input type="text" name="uname" value=""></td>
								                <td><input type="text" name="email" value=""><input type="button" value="Фильтр" /></td>
								                <td></td>
								                <td></td>
								                <td></td>
								                <td></td>
								        	...
								        	<?php foreach ($data->list_player as $row): ?>
												<tr class="items<?php if ($row === end($data->list_player)) echo ' last' ?>">
								                    <td width="29">&nbsp;&nbsp; <input type="checkbox" name="id[]" value="<?php echo $row->users_id ?>" /></td>
								                    <td>
								                    	<strong><a href="#edit" title="<?php echo $row->username ?>"><?php echo $row->name ?></a></strong>
								                    	(<?php echo $row->username ?>)								                    	
								                    </td>
								                    <td><a href="mailto:<?php echo $row->email ?>"><?php echo $row->email ?></a></td>
								                    <td><?php echo $row->code ?></td>
								                    <td align="right"><?php printf("%.2f", $row->add_cash); ?></td>
								                    <td align="right"><?php printf("%.2f", $row->user_loss) ?></td>
								                    <td align="right"><?php printf("%.2f", $row->user_balance) ?></td>
								                    <td><a href="/finance/cashout/filter/<?php echo $row->users_id ?>,0,0,0,0">Всего <?php echo $row->count_cashout ?></a></td>
								                </tr>
											<?php endforeach ?>												
								            </table>		
